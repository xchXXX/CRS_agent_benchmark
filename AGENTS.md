...
实施任何改动前必须检查当下的文档内容和测试是否需要修改，如果有则必须先修改同步文档和测试，最后实施修改
查找信息时按照渐进式披露的范式阅读信息，不一次性查找出所有的信息。
各个方面的实现环节，都需要满足当下的最佳实践，具体的最佳实践可自由查阅网络资料。

## 外部仓库导入规则（强制）
- 当用户要求“把其他仓库 / 其他远端分支的内容放进某个子目录”时，禁止在当前仓库直接执行整仓 `git switch`、`git checkout`、`git reset`、`git pull` 到该外部分支。
- 只要目标分支与当前仓库存在“无共同历史”风险，或需求本身明确指向“仅更新某个子目录”，必须使用“临时目录克隆/归档 + 子目录同步”方案。
- 标准流程固定为：
  1. 先确认当前仓库根目录与目标子目录，明确同步边界。
  2. 将外部仓库或外部分支单独拉取到工作区外或临时目录。
  3. 只把需要的内容同步到目标子目录，禁止影响仓库根目录其他模块。
  4. 同步完成后检查变更范围，确认没有把根目录或其他模块一起改动。
- 如果看到 `warning: no common commits`、外部仓库地址不同、或用户明确要求“只放进某个目录”，一律按上述流程处理，不得再尝试整仓切分支。
