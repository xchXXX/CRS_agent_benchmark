...
3. 运行前检查 `.local/benchmark_app_token.txt` 是否存在且非空
4. 运行前检查 backend `.env/.env.runtime` 中 OpenRouter 配置是否可用
5. 运行前先走代理直连 OpenRouter 做一次 TLS/HTTP 预检
6. 用户模拟阶段强制使用 fresh client，不复用长生命周期兼容客户端
7. 用户模拟阶段显式加长超时
8. 用户模拟阶段对 `SSL EOF`、超时、连接中断等传输类异常做自动重试
9. backend 与 benchmark 子进程统一继承同一套代理和模型环境
10. 每次 one-click 都有独立日志目录，避免后续运行覆盖前一次证据
11. image probe 使用轻量化固定提示，不再直接拿复杂真实 case 问题做探测
...
2. 如本轮配置会用到 OpenRouter，再做 `openrouter transport preflight`
3. 只有预检通过才真正启动 backend / benchmark
4. image probe 调用图文接口时：
   - 固定只带 1 张图片
...
5. 用户模拟调用 OpenRouter 时：
   - fresh client
   - 默认超时 `600s`
   - 默认传输重试 `4` 次
   - 默认线性退避 `2s / 4s / 6s ...`
6. OpenRouter preflight 自身也会做短退避重试，不会因为单次瞬时 EOF 就直接误判失败
7. 如果 4 次传输重试后仍失败，才把问题记成真实运行错误，并把最后错误落到日志
...
2. `.local/benchmark_app_token.txt` 已生成
3. backend `.env` / `.env.runtime` 中的 OpenRouter key 可用
4. 若你在国内环境，`openrouter.ai` 必须被代理接管，建议全局代理或强规则代理

其中 1-3 现在都会由 `run.py --one-click` 自动检查；代理未就绪时会先尝试自动启动 SakuraCat。
...
### 7.3 `error_http` 里仍出现 `OpenrouterException`
...
### 7.4 `existing_backend_probe_failed`
...
2. 看 `proxy_probe` / `openrouter_preflight`
3. 看 `benchmark/reports/one_click_logs/<one_click_run_id>/*.stderr.log`
4. 看 `benchmark/reports/runs/<run_id>/runtime.log`
5. 看 `benchmark/reports/runs/<run_id>/report.score.json`
